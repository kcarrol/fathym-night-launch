...
const tools = [{
  icon: Cpu,
  label: "Code generation frameworks and assistants"
}, {
  icon: Cloud,
  label: "Cloud platforms and infrastructure tooling"
}, {
  icon: GitBranch,
  label: "CI/CD pipelines and deployment automation"
}, {
  icon: Bot,
  label: "Agent frameworks and AI orchestration tools"
}];
const missingPieces = [{
  icon: FileQuestion,
  label: "What the system consists of"
}, {
  icon: Server,
  label: "Where it runs"
}, {
  icon: KeyRound,
  label: "How identity, data, and environments are scoped"
}, {
  icon: History,
  label: "How change is allowed to happen over time"
}];
  ...
  return <section className="py-32 relative overflow-hidden">
      ...
      <div className="container relative z-10 mx-auto px-6">
        <div className="max-w-4xl mx-auto text-center mb-16">
          ...
          <p className="text-lg text-muted-foreground">Modern software stacks are made up of powerful tools.</p>
        </div>

        {/* Tools Grid */}
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
          {tools.map((tool, index) => <div key={tool.label} className="group relative p-5 rounded-lg border border-border bg-card/30 backdrop-blur-sm hover:border-neon-cyan/50 transition-all duration-300" style={{
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-12">
            {missingPieces.map((piece, index) => <div key={piece.label} className="flex items-center gap-4 p-4 rounded-lg border border-border/50 bg-muted/20" style={{
            animationDelay: `${index * 0.1}s`
          }}>
