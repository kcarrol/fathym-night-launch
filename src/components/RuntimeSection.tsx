...
const runtimes = [
  { name: "Web", icon: Box, color: "neon-cyan" },
  { name: "API", icon: Cpu, color: "neon-magenta" },
  { name: "Workflow", icon: Workflow, color: "neon-lime" },
  { name: "AI", icon: GitBranch, color: "neon-yellow" },
];

const RuntimeSection = () => {
  return (
    <section className="py-32 relative overflow-hidden border-y border-border/50">
      ...
      <div className="container relative z-10 mx-auto px-6">
        <div className="max-w-3xl mx-auto text-center mb-16">
          <h2 className="font-display text-3xl md:text-5xl font-bold tracking-tight mb-6">
            Modular Runtimes,
            <br />
            <span className="text-gradient-magenta">Not Monoliths</span>
          </h2>
          <p className="text-lg text-muted-foreground">
            EaC separates system definition from system execution. Execution happens through modular, 
            composable runtimes built from shared processors.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {runtimes.map((runtime) => (
            <div
              key={runtime.name}
              className="group relative p-8 rounded-xl border border-border bg-card/20 backdrop-blur-sm text-center hover:border-primary/50 transition-all duration-300"
            >
