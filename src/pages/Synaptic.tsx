...
const gapItems = [
  ...
  "Coordinating steps and state",
  "Managing agent flows",
];

const structuralGaps = [
  "AI workflows lack system-level context",
  "Execution boundaries are implicit",
  "Identity and tenancy are bolted on",
  "Deployment, isolation, and portability are delegated to infrastructure",
  "Reuse across systems is limited",
];
...
const frameworkAdds = [
  ...
  "Runtime composition and execution context",
  "Identity and tenancy scoping",
  "Policy enforcement",
  "System-level portability",
];

const agentRules = [
  "Operate only within declared capabilities",
  "Access data through governed interfaces",
  "Execute under explicit identity and tenant scopes",
  "Produce auditable actions, decisions, and outcomes",
];

const mcpFeatures = [
  "Available actions are explicitly declared",
  "Context is scoped and governed",
  "AI tools can discover and use capabilities safely",
];

const portabilityEnvironments = [
  { icon: Server, text: "In cloud environments" },
  { icon: Globe, text: "At the edge" },
  { icon: Layers, text: "Alongside application runtimes" },
  { icon: Shield, text: "In regulated systems" },
];

const executionSteps = [
  "Everything as Code defines system intent",
  "Stewardship validates intent, policy, and scope, then resolves the execution graph",
  "Synaptic workflows are composed from declared processors",
  "AI agents execute within governed runtime boundaries",
  "Outputs and proposed changes flow back through stewardship",
];

const whenToUse = [
  "AI is part of the architecture, not an experiment",
  "Workflows must be reusable and evolvable",
  "Access control and auditability matter",
  "Multi-tenant isolation is required",
  "AI interacts with real data and real services",
];
        ...
        <div className="container relative z-10 mx-auto px-6">
          <div className="max-w-4xl">
            ...
            <p className="text-lg text-muted-foreground max-w-3xl">
              AI is no longer limited to generating code. It now executes workflows, reasons over live data, 
              makes decisions, and proposes changes to running systems. The challenge is no longer enabling 
              AI execution. The challenge is enabling AI execution as part of a system. Synaptic exists to 
              make AI execution composable, governed, and system-aware.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-card/30">
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {synapticProvides.map((item, index) => (
              <div 
                key={index}
                className="p-6 rounded-xl border border-border bg-background/50 hover:border-secondary/50 transition-colors"
              >
      ...
      <section className="py-24 bg-card/30">
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {portabilityEnvironments.map((item, index) => (
              <div 
                key={index}
                className="p-6 rounded-xl border border-border bg-background/50 text-center hover:border-primary/50 transition-colors"
              >
          ...
          <div className="max-w-4xl mx-auto">
            <div className="space-y-4">
              {executionSteps.map((step, index) => (
                <div 
                  key={index}
                  className="flex items-center gap-6 p-6 rounded-xl border border-border bg-card/50"
                >
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-12">
              {whenToUse.map((item, index) => (
                <div 
                  key={index}
                  className="flex items-center gap-4 p-6 rounded-xl border border-secondary/30 bg-secondary/5"
                >
